...
+
Sie können die VMware vSphere Web Client GUI in vCenter für alle Backup- und Restore-Vorgänge von VMware Virtual Machines (traditionelle VMs und vVol VMs), VMDKs und Datastores nutzen. Für vVol VMs (VMs in vVol Datastores) werden nur absturzkonsistente Backups unterstützt. Sie haben auch die Möglichkeit, VMs und VMDKs wiederherzustellen sowie Dateien und Ordner auf dem Gastbetriebssystem wiederherzustellen.
...
* ONTAP Tools für VMware vSphere sind für den Schutz von VMware vVol VMs erforderlich.
+
Mit den ONTAP Tools können Sie Storage für VVols in ONTAP und im VMware Web-Client bereitstellen und konfigurieren.
